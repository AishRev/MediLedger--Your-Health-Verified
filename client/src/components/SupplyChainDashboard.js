...
const SupplyChainDashboard = ({ contract, setView }) => {
    const [manufacturerView, setManufacturerView] = useState('loading');
    ...
    const renderDashboardView = () => (
        <>
            <div className="dashboard-nav">
                <button className={`nav-tab ${nav === 'profile' ? 'active' : ''}`} onClick={() => setNav('profile')}>Profile</button>
                <button className={`nav-tab ${nav === 'add' ? 'active' : ''}`} onClick={() => setNav('add')}>Add Drug</button>
                <button className={`nav-tab ${nav === 'track' ? 'active' : ''}`} onClick={() => setNav('track')}>Track Drug</button>
            </div>
            <div className="dashboard-content">
                {nav === 'profile' && (
                    <div className="dashboard-section">
    ...
    return (
        <div className="dashboard">
            ...
            {manufacturerView === 'loading' && <div className="dashboard-content"><p>Loading information...</p></div>}
            {manufacturerView === 'register' && renderRegisterView()}
            {manufacturerView === 'dashboard' && renderDashboardView()}
        </div>
    );
